...
LeggedInterface::LeggedInterface(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
  ...
  loadData::loadCppDataType(taskFile, "selfCollision.enabled", selfCollisionEnabled_);
...
void LeggedInterface::setupOptimalControlProblem(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
  ...
  for (size_t i = 0; i < centroidalModelInfo_.numThreeDofContacts; i++) {
    ...
    problemPtr_->costPtr->add(footName + "_footTrajectoryTracking",
                              getFootTrajectoryTrackingCost(*eeKinematicsPtr, i, taskFile, verbose));
  ...
  // Self-collision geometry parsing can be disabled for simulation-only data generation.
...
void LeggedInterface::setupPreComputation(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
                                          bool verbose) {
  const auto footTrajectoryConfig = loadBezierFootTrajectoryConfig(taskFile, "foot_trajectory_tracking", verbose);
  const auto basePose = centroidal_model::getBasePose(initialState_, centroidalModelInfo_);
  const matrix3_t worldToBase = getRotationMatrixFromZyxEulerAngles(vector3_t(basePose.tail<3>())).transpose();
  std::vector<vector3_t> nominalFootOffsetsInBase;
  nominalFootOffsetsInBase.reserve(centroidalModelInfo_.numThreeDofContacts);
  for (size_t i = 0; i < centroidalModelInfo_.numThreeDofContacts; i++) {
    const auto eeKinematicsPtr = getEeKinematicsPtr({modelSettings_.contactNames3DoF[i]}, modelSettings_.contactNames3DoF[i]);
    nominalFootOffsetsInBase.push_back(worldToBase * (eeKinematicsPtr->getPosition(initialState_).front() - basePose.head<3>()));
  }
  problemPtr_->preComputationPtr = std::make_unique<LeggedRobotPreComputation>(
      *pinocchioInterfacePtr_, centroidalModelInfo_, *referenceManagerPtr_->getSwingTrajectoryPlanner(), modelSettings_,
      footTrajectoryConfig, nominalFootOffsetsInBase);
}
...
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<StateInputCost> LeggedInterface::getFootTrajectoryTrackingCost(const EndEffectorKinematics<scalar_t>& eeKinematics,
                                                                               size_t footIndex, const std::string& taskFile,
                                                                               bool verbose) {
  const auto config = loadFootTrajectoryTrackingCostConfig(taskFile, "foot_trajectory_tracking", verbose);

  const auto basePose = centroidal_model::getBasePose(initialState_, centroidalModelInfo_);
  const matrix3_t worldToBase = getRotationMatrixFromZyxEulerAngles(vector3_t(basePose.tail<3>())).transpose();
  const vector3_t nominalFootOffsetInBase = worldToBase * (eeKinematics.getPosition(initialState_).front() - basePose.head<3>());

  return std::make_unique<FootTrajectoryTrackingCost>(*referenceManagerPtr_, eeKinematics, footIndex, centroidalModelInfo_,
                                                      nominalFootOffsetInBase, config);
}
