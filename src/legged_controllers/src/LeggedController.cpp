...
// Modified for legged_mpc_amp: added AMP data logging, configurable joint names
//
...
bool LeggedController::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& controller_nh) {
  ...
  if (leggedInterface_->hasSelfCollision()) {
  ...
  if (!controller_nh.getParam("/joint_names", joint_names)) {
    joint_names = leggedInterface_->modelSettings().jointNames;
    ROS_WARN("[LeggedController] /joint_names not set, using model_settings.jointNames");
  }
  ...
  if (!controller_nh.getParam("/imu_name", imu_name)) {
    imu_name = "imu";
    ROS_WARN("[LeggedController] /imu_name not set, using default: %s", imu_name.c_str());
  }
  imuSensorHandle_ = robot_hw->get<hardware_interface::ImuSensorInterface>()->getHandle(imu_name);
  ...
  if (enableAmpLogging_) {
    ...
    controller_nh.getParam("/amp_start_recording", ampConfig.startRecording);
    ...
    ROS_INFO("[LeggedController] AMP logger enabled. Log dir: %s, start recording: %d",
             ampConfig.logDir.c_str(), ampConfig.startRecording);
  }
...
void LeggedController::update(const ros::Time& time, const ros::Duration& period) {
  ...
  if (enableAmpLogging_ && ampDataLogger_) {
    ampDataLogger_->log(currentObservation_.time, optimizedState, optimizedInput, x, measuredRbdState_,
                        plannedMode, leggedInterface_->getCentroidalModelInfo());
  }
